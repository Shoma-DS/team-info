...
- 台本を読んだあと、シーンごとに必要な画像の内容を決め、次のいずれかで新規素材を用意する。
  - 台本に合わせた画像生成
  - いらすとや等の利用条件を満たすイラスト素材
  - フリー素材サイトの利用条件を満たす写真・イラスト素材
- 素材は動画ごとの専用フォルダへ保存し、ファイル名は用途が分かる英数字名にする。既存動画の `public/assets` や `public/images` の別プロジェクト配下から直接参照しない。
- 実装前に「台本要点 -> 必要画像 -> 取得/生成方法 -> 保存先」を短く整理し、画像ごとにこの動画専用であることを確認する。
- 既存ショートのコンポーネントをベースにする場合も、画像配列・import・staticFile の参照先は必ず新規素材へ差し替える。過去作と同じ画像が残っていないか最終確認する。
...
- テンプレート専用スキル:
  - `.agent/skills/remotion/video-production/remotion-template-*/`（sleep_travel系）
  - `.agent/skills/remotion/video-production/acoriel/remotion-template-*/`（acoriel系）
