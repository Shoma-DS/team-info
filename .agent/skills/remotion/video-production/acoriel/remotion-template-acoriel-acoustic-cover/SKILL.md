...
- ユーザーに素材の配置先を案内するときも、`"$TEAM_INFO_ROOT/..."` の形で表示する。
...
```bash
ffmpeg -i "/absolute/path/to/<入力ファイル>.wav" -q:a 2 "$TEAM_INFO_ROOT/Remotion/my-video/public/assets/channels/acoriel/songs/<曲名>.mp3"
```
...
```bash
ls -lh "$TEAM_INFO_ROOT/Remotion/my-video/public/assets/channels/acoriel/songs/<曲名>.mp3"  # 5MB前後になっていること
```
...
3. **背景素材**（テンプレートによって異なる）
   ...
   - `MultiBG` の場合:
     ...
     2. ユーザーに以下を表示して待機する:
        ...
        配置先: "$TEAM_INFO_ROOT/Remotion/my-video/public/assets/songs/[曲名]/"
...
## Remotion実装ルール（必須）
- 背景画像、背景動画、歌詞字幕、補助字幕、効果音など、同じ種類で時系列が重ならない素材は、種類ごとに `<Sequence>` を1本へ統合する。
...
- 既存コードでフォントを差し替える際、`npm --prefix "$TEAM_INFO_ROOT/Remotion/my-video" run lint` を必ず通す。
...
4.3. **（MultiBG のみ）背景動画を事前合成する**（必須）
   - まず音源の実尺を取得する（これを `--total-sec` に使う）:
     ...
     ffprobe -v quiet -print_format json -show_format \
       "$TEAM_INFO_ROOT/Remotion/my-video/public/assets/songs/[曲名]/audio.mp3" \
   ...
   - 取得した秒数を確認したら、**自分では実行せず**、以下のコマンドをユーザーに提示して**任意のターミナルから実行**するよう案内して待機する:
     ```bash
     python "$TEAM_INFO_ROOT/.agent/skills/common/scripts/team_info_runtime.py" run-remotion-python -- "$TEAM_INFO_ROOT/Remotion/scripts/prerender_bg_video.py" \
       --output "$TEAM_INFO_ROOT/Remotion/my-video/public/assets/songs/[曲名]/bg_prerendered.mp4" \
       --crossfade-sec 1.5 \
       --total-sec [音源の秒数] \
       --fps 30 --width 1920 --height 1080 \
       --ping-pong \
       "$TEAM_INFO_ROOT/Remotion/my-video/public/assets/songs/[曲名]/*.mp4"
     ```
   - 完了したら「終わりました」と伝えるよう案内する。
   - ユーザーから完了報告が得られるまで次工程に進まない。
