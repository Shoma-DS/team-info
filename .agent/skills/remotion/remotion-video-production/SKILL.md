...
- レンダリングコマンドを見せるときは、出力先も `/Users/deguchishouma/team-info/outputs/...` にする。
...
- `acoriel/acoustic_cover` -> `remotion-template-acoriel-acoustic-cover`
...
## レンダリング確認ルール（必須）
- `cd /Users/deguchishouma/team-info/Remotion/my-video && npx remotion render ...` は、ユーザーの明示承認があるまで実行しない。
- レンダリング前は必ず次の文言で確認する:
  - `出力しますか？書き出しますか？`
