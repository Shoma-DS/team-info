...
9. レンダリング完了後、以下のコマンドで Google Drive にコピーする（コマンドをユーザーに提示するだけ・自分では実行しない）：
   ```bash
   mkdir -p "/Users/deguchishouma/Library/CloudStorage/GoogleDrive-syouma1674@gmail.com/マイドライブ/team-info/outputs/寝ながらトラベル" && cp "$TEAM_INFO_ROOT/outputs/sleep_travel/renders/[ファイル名].mp4" "/Users/deguchishouma/Library/CloudStorage/GoogleDrive-syouma1674@gmail.com/マイドライブ/team-info/outputs/寝ながらトラベル/"
   ```
