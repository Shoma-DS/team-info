...
description: 台本をVOICEVOXで音声化する実行フロー。MCPツールを活用し、スピーカー確認・テスト音声・本番生成を行う。Python は Docker ランタイム、音声合成は GUI なしの VOICEVOX Engine を使う。
---
...
- Python 実行は `python "$TEAM_INFO_ROOT/.agent/skills/common/scripts/team_info_runtime.py" run-remotion-python -- ...` を使う。
- Python 依存は Docker ランタイム `team-info/python-skill-runtime:3.11.9` に固定する。
- VOICEVOX は GUI アプリではなく Docker 上の Engine を使い、`http://127.0.0.1:50021` で待ち受ける。
...
- **存在しない場合（初回）**:
  1. 次のコマンドを実行する。
     ...
     ```
  2. 次のコマンドで GUI なしの VOICEVOX Engine を起動する。
     ```bash
...
- まず次のコマンドで状態を確認する。
  ...
  ```
- `stopped` の場合は、先に `start-voicevox-engine` を実行する。
- その後に `voicevox_get_speakers` を実行して接続を確認する。
...
## 失敗時の扱い
- `voicevox_generate_full` 実行時に Docker ランタイム不足で失敗した場合は、初回扱いとして `python "$TEAM_INFO_ROOT/.agent/skills/common/scripts/team_info_runtime.py" build-remotion-python` を提案する。
- VOICEVOX接続エラー時は `python "$TEAM_INFO_ROOT/.agent/skills/common/scripts/team_info_runtime.py" start-voicevox-engine` を案内する。
