...
- Python 実行は `python /Users/deguchishouma/team-info/.agent/skills/common/scripts/team_info_runtime.py run-remotion-python -- ...` を使う。
...
## 失敗時の扱い
- `voicevox_generate_full` 実行時に `.venv` 不足や依存不足で失敗した場合は、初回扱いとして `python /Users/deguchishouma/team-info/.agent/skills/common/scripts/team_info_runtime.py run-remotion-python -- -m pip install requests` を提案し、承認があれば実行する。
