...
- 発表用途では、サムネ一覧で流れが読めること、冒頭で掴めること、1ページ1メッセージであることも同時に満たす。
...
5. 1ページに 1 メッセージを置く。見出しで興味を作り、補足は 2 から 3 点までに留める。
6. 数字、事例、人物、引用など「温度がある材料」は、本文へ埋めずに独立ページとして扱うと読みやすい。
...
### Phase 3-b: YouTube 的な見せ方を混ぜる
1. 冒頭 3 ページ以内で次を見せる。
- 誰向けの資料か
- 何が変わるか
- なぜこの先を読む価値があるか
2. 1ページ1メッセージを徹底し、話す文章をそのまま貼らない。
3. サムネ一覧だけ見ても流れが追えるように、見出しは独立して意味が通る短文にする。
4. 基本の流れは `hook → context → evidence → example → takeaway → CTA` とし、説明ページだけを連続させない。
5. 数字、比較、実例、ビフォーアフターは、本文より先に目に入る位置へ置く。
6. 強いページと静かなページを交互に作り、全ページを同じテンションにしない。
...
- 冒頭 3 ページを前置きだけで消費しない。
- 見出しを読んでも要点が分からない抽象語だけのページを量産しない。
- 話す原稿をそのまま段落で貼らない。
- 説明ページだけを長く連続させない。2 から 4 ページごとに実例、比較、要点回収を挟む。
- サムネイル表示で主役が分からないページを作らない。
...
- サムネ一覧だけ見ても、話の流れが追えるか。
- 冒頭 3 ページで対象、価値、結論の方向が見えるか。
- 無音でめくっても骨子が分かるか。
- 説明、証拠、実例、要点回収のバランスが偏っていないか。
...
5. ページ一覧表示にして、見出しだけでもストーリーが読めるか確認する。
