...
- 転職ショートごとに、その台本専用の画像を新規生成または新規取得する。過去の転職ショートで使った人物・オフィス・履歴書・悩む会社員などの画像素材を流用しない
- 過去作のサイズ感や配置は参考にしてよいが、画像そのもの、素材フォルダ、生成画像セット、staticFile 参照は必ず差し替える
...
- 取得後は `metadata.json` と素材ファイル一覧を見て、今回の動画フォルダ内に新規取得素材だけが入っていることを確認する
