...
- リポジトリ内ファイルは `/Users/deguchishouma/team-info/...` の形で示す。
- ユーザー指定の入力先・出力先も `"[入力動画の絶対パス]"` `"[出力先の絶対パス]"` の形で案内する。
...
```bash
cd "[出力先の絶対パス]/remotion" && npx tsc --noEmit
```
...
- プレビューコマンド（絶対パス）:
  ```bash
  cd "[出力先の絶対パス]/remotion" && npx remotion studio
  ```
- レンダリングコマンド:
  ```bash
  cd "[出力先の絶対パス]/remotion" && npx remotion render --composition=Viral-[platform]-[date] --output="[出力先の絶対パス]/output.mp4"
  ```
