...
## 絶対パスルール（必須）
- ユーザーにコマンドを渡すときは、対象プロジェクトの絶対パスを必ず入れる。
- 例: `/absolute/path/to/project` を省略せずに書く。

## 環境情報
- **OS**: macOS（Intel x86_64）
- **Python**: 3.11.9（uv 経由でインストール）
...
```bash
# 依存関係の同期
cd /absolute/path/to/project && uv sync

# スクリプトの実行
cd /absolute/path/to/project && uv run /absolute/path/to/project/<script.py>
```
