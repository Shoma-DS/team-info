...
**Never create files in the agent workspace.** Use `/tmp/` for temporary output and `~/.config/team-info/agent-reach/` for persistent data.
...
```bash
cd ~/.config/team-info/agent-reach/tools/wechat-article-for-ai && python3 main.py "https://mp.weixin.qq.com/s/ARTICLE_ID"
```
...
```bash
# 转录单集播客（输出文本到 /tmp/）
~/.config/team-info/agent-reach/tools/xiaoyuzhou/transcribe.sh "https://www.xiaoyuzhoufm.com/episode/EPISODE_ID"
```
