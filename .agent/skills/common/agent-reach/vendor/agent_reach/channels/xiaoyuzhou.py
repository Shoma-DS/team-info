...
from agent_reach.paths import get_xiaoyuzhou_tools_dir
...
class XiaoyuzhouChannel(Channel):
    ...
    def check(self, config=None):
        ...
        # Check script exists
        script = str(get_xiaoyuzhou_tools_dir() / "transcribe.sh")
        if not os.path.isfile(script):
            return "off", (
                "转录脚本未安装。运行：\n"
                "  agent-reach install --env=auto\n"
                f"  或手动复制 transcribe.sh 到 {get_xiaoyuzhou_tools_dir()}/"
            )
