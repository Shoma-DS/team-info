...
def main():
    ...
    p_uninstall.add_argument("--keep-config", action="store_true",
                             help="Remove skill files only, keep team-info Agent Reach config and tokens")
...
def _install_skill():
    ...
    installed = False
    for target in get_skill_targets():
        try:
            os.makedirs(target, exist_ok=True)
            with open(Path(target) / "SKILL.md", "w", encoding="utf-8") as f:
                f.write(skill_md)
            platform_name = "OpenClaw" if "openclaw" in str(target).lower() else "Agent"
...
def _uninstall_skill():
    ...
    removed = False
    for skill_path in get_skill_targets():
        platform_name = "OpenClaw" if "openclaw" in str(skill_path).lower() else "Agent"
...
def _install_xiaoyuzhou_deps():
    ...
    tools_dir = str(get_xiaoyuzhou_tools_dir())
...
def _configure_xhs_cookies(value):
    ...
    if not docker:
        ...
        cookie_path = str(get_xhs_cookie_path())
...
def _cmd_uninstall(args):
    ...
    # ── 1. Config directory (team-info managed) ──
    ...
    # ── 2. Skill files ──
    for skill_path in get_skill_targets():
        if ".openclaw" in str(skill_path):
            platform_name = "OpenClaw"
        elif ".claude" in str(skill_path):
            platform_name = "Claude Code"
        elif ".agents" in str(skill_path):
            platform_name = "Agent"
        else:
