...
- Discord の詳細メッセージには Zoom リンクに加えてミーティングIDも記載する
...
- 認証方式は `file` を推奨するが、実行時は `file` / `keyring` のうち有効な `gws` 認証を自動選択する
...
| `gws` で認証エラー | backend 不一致 / 保存済み資格情報なし | まず `gws auth status` を確認し、必要なら `GOOGLE_WORKSPACE_CLI_KEYRING_BACKEND=file gws auth login -s sheets,drive,calendar` を実行 |
