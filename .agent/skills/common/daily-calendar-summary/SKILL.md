...
実行前に Webhook URL を環境変数に設定する:

```bash
export DISCORD_DAILY_WEBHOOK="https://discord.com/api/webhooks/1489635669415297024/GHErD1DE20qHexSTtZuNYUJz4erjEUCnRra_LrJF7Sz394UjWfMC0Gp9VoIkvM5yGgVT"
```

以下のBashコマンドを実行する（JSONは実際のイベントデータで置き換えること）:

```bash
python3 - <<'PYEOF' | python3 "$TEAM_INFO_ROOT/scripts/daily_calendar_summary.py"
import json
events = [
    # 収集したイベントデータをここに入れる
    # 例:
