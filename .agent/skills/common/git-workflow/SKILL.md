...
## 適用範囲（必須）
- ユーザーが `/git` や `/git-nd` を明示しなくても、`pushして`、`プッシュして`、`PR作って`、`反映して`、`リモートに上げて`、`コミットして送って` のように Git 反映を依頼した場合は、このスキルを必ず適用する。
- `git push` を実行する前に、必ずこのファイルの「オーナー機とPR分岐」と「Git LFS 無料枠ルール」を読み直す。
- `main` ブランチ上で作業しているときほど慎重に扱う。`owner-status` が `other` の場合は、たとえユーザーが「push」とだけ言っても `main` へ直接 push せず、作業ブランチから PR を作る。
- `git push origin main` を実行してよいのは、同じ作業内で `owner-status` の結果が `owner` だと確認できた場合だけ。
