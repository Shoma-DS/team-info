...
```bash
ffmpeg -i "/absolute/path/to/<入力ファイル>.wav" -q:a 2 "/Users/deguchishouma/team-info/Remotion/my-video/public/assets/channels/acoriel/songs/<曲名>.mp3"
```
...
```bash
ls -lh /Users/deguchishouma/team-info/Remotion/my-video/public/assets/channels/acoriel/songs/<曲名>.mp3  # 5MB前後になっていること
```
...
3. **背景素材**（テンプレートによって異なる）
   ...
   - `MultiBG` の場合:
     ...
     2. ユーザーに以下を表示して待機する:
        ...
        配置先: /Users/deguchishouma/team-info/Remotion/my-video/public/assets/songs/[曲名]/
...
- 既存コードでフォントを差し替える際、`npm --prefix /Users/deguchishouma/team-info/Remotion/my-video run lint` を必ず通す。
...
4.3. **（MultiBG のみ）背景動画を事前合成する**（必須）
   - まず音源の実尺を取得する（これを `--total-sec` に使う）:
     ...
     ffprobe -v quiet -print_format json -show_format \
       /Users/deguchishouma/team-info/Remotion/my-video/public/assets/songs/[曲名]/audio.mp3 \
   ...
   - 取得した秒数を確認したら、**自分では実行せず**、以下のコマンドをユーザーに提示して**任意のターミナルから実行**するよう案内して待機する:
     ```bash
     python /Users/deguchishouma/team-info/.agent/skills/common/scripts/team_info_runtime.py run-remotion-python -- /Users/deguchishouma/team-info/Remotion/scripts/prerender_bg_video.py \
...
7. `npm --prefix /Users/deguchishouma/team-info/Remotion/my-video run lint` を実行し、エラーを解消する。
