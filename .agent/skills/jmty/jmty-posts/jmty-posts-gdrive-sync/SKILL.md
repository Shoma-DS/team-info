...
## 実行コマンド
```bash
python "$TEAM_INFO_ROOT/.agent/skills/jmty/jmty-posts-gdrive-sync/scripts/sync_jmty_posts_to_gdrive.py"
```

## 引数でコピー先を変更する場合
```bash
python "$TEAM_INFO_ROOT/.agent/skills/jmty/jmty-posts-gdrive-sync/scripts/sync_jmty_posts_to_gdrive.py" "/destination/root/absolute/path"
```
