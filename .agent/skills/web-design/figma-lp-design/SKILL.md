...
インストールコマンド（ユーザーが実行）：
```bash
cd "$TEAM_INFO_ROOT/personal/deguchishouma/outputs/lp-design/[プロジェクト名]" && npx getdesign@latest add [テンプレート名]
...
**出力先（ユーザーが実行）**
```bash
mkdir -p "$TEAM_INFO_ROOT/personal/deguchishouma/outputs/lp-design/[プロジェクト名]/assets"
# DESIGN.md を出力フォルダにもコピーしておく
cp "$TEAM_INFO_ROOT/personal/deguchishouma/outputs/lp-design/[プロジェクト名]/DESIGN.md" \
   "$TEAM_INFO_ROOT/personal/deguchishouma/outputs/lp-design/[プロジェクト名]/assets/"
```

Google Drive へのコピー：
```bash
rclone copy "$TEAM_INFO_ROOT/personal/deguchishouma/outputs/lp-design/[プロジェクト名]/" "gdrive:1QKaUP9fvA46mINkpSR1b2wqrIBE6By0t/personal/deguchishouma/outputs/lp-design/[プロジェクト名]/" --progress
```
