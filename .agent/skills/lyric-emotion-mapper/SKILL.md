...
アカウントに応じて `Remotion/video_resources/channels/<account>/assets/` を参照する。
...
```
音源ディレクトリ: Remotion/video_resources/channels/acoriel/assets/songs/
```
...
```
背景画像ディレクトリ: Remotion/video_resources/channels/<account>/assets/backgrounds/
```
...
```
エフェクトテンプレート: Remotion/video_resources/channels/<account>/effects/templates/
```
...
```
歌詞ディレクトリ: Remotion/video_resources/channels/acoriel/assets/lyrics/
```
...
**歌詞ファイルの命名規則:**
- 音源と同じ名前で `.txt` 拡張子
- 例: 音源が `LA LA LA LOVESONG.wav` → 歌詞は `LA LA LA LOVESONG.txt`
...
選択した音源を Remotion の public/assets にコピーする:
```bash
cp "<選択した音源パス>" Remotion/my-video/public/assets/audio.wav
```
...
Phase 3: 音源をコピー → Remotionプレビュー（ローカル確認）→ レンダリング（承認後のみ）
```

## ディレクトリ構成

```
Remotion/video_resources/channels/acoriel/assets/
├── songs/          # 音源ファイル (.wav, .mp3)
├── lyrics/         # 歌詞テキスト (.txt)
└── backgrounds/    # 背景画像

Remotion/my-video/public/assets/
├── audio.wav                    # 現在使用中の音源（コピー）
