...
Loom 認証情報の正本は `team-info/.env` とし、Claude Code / Codex のどちらでも同じ `.env` を参照する。
Neon の接続先も `NEON_DATABASE_URL` を `.env` または環境変数へ置き、クライアント固有の設定ファイルは互換用の mirror としてのみ扱う。
...
| Neon(Postgres) upsert | Python（run_all.py） | 既存分類結果とまとめて登録できる |
...
他のファイルはあれば一緒に Neon へ入る。

---

## STEP 2: export を分類・保存・Neon 登録する

### 前提：`.env` に Neon の接続情報を入れる

```bash
echo 'NEON_DATABASE_URL=postgresql://<user>:<password>@<host>/<db>?sslmode=require' >> "$TEAM_INFO_ROOT/.env"
echo 'NEON_TABLE=sales_coaching_transcripts' >> "$TEAM_INFO_ROOT/.env"
```

`run_all.py` はシェル環境変数だけでなく、repo 直下の `.env` に入れた `NEON_DATABASE_URL` も自動で読む。
Neon は通常の PostgreSQL 接続として扱う。
...
複数件ある場合は `--loom-export-dir` を複数回付ける。旧オプション `--sync-supabase` / `--supabase-table` も互換用に残してあるが、新規運用では使わない。
...
## STEP 3: Neon(Postgres) の最小テーブル例

`ON CONFLICT (loom_video_id)` で upsert するので、`loom_video_id` に unique 制約が必要。
...
| `search_loom.py` | `.env` の `LOOM_COOKIE` を使って Loom を検索し、動画詳細と transcript を直接取得する | なし |
| `classify.py` | キーワードベースで種別・話者を簡易判別 | なし |
| `save_transcript.py` | Loom export の読込、文字起こし保存、Neon payload 作成 | なし |
| `run_all.py` | Loom MCP export の取り込みと Neon upsert の実行 | なし |

---

## 候補者名しか分からないときの取得フロー

候補者名や動画タイトルの一部だけ分かっている場合は、まず `.env` の `LOOM_COOKIE` を使う補助スクリプトで Loom を検索する。
...
### 2. video_id が分かったら transcript を取る
...
これで `/tmp/loom-mcp/<loom_video_id>/metadata.json` と `transcript.txt` が作られる。

### 3. 必要なら sales-coaching の保存フローへ渡す

```bash
"$TEAM_INFO_ROOT/Remotion/.venv/bin/python3.11" \
  "$TEAM_INFO_ROOT/.agent/skills/sales/sales-coaching/scripts/run_all.py" \
  --loom-export-dir "/tmp/loom-mcp/<loom_video_id>"
```
...
この場合は `.env` に `LOOM_CLIENT_ID` と `LOOM_CLIENT_SECRET` が必要。
