...
- `loom_video_id`
- `loom_url`（台本に記載する正本。IDしかない場合は `https://www.loom.com/share/<loom_video_id>` に変換する）
- 動画タイトル
- 録画日
- 担当者名（一次面談で実際に対応したスタッフ名）
- 現クロージング担当者名（未指定なら `出口` を既定値とする）
...
- `## 担当: <現クロージング担当者名>（二次） / <担当者名>（一次）  loom_url: <https://www.loom.com/share/...>`
- `## 参考: 営業ロープレフォーマット.pdf`
- `## 候補者プロファイルサマリー`
- `**最大のフック**: ...`
- `## 2次面接 台本（全フェーズ）`
...
- `現クロージング担当者名` は原則として二次面接で実際に話す人として扱い、未指定なら `出口` を既定値にする
- 既定値の `出口` を使う場合は、「代表の僕として」「僕は代表として」のように、代表本人が判断と責任を持って話しているトーンをフェーズ0・6・7に自然に入れる
...
- 販売台本の冒頭メタ情報には、Loom の ID 単体ではなく、必ず開ける Loom URL を `loom_url: https://www.loom.com/share/<loom_video_id>` の形で記載する
