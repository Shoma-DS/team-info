/**
 * 「（新）」タイトルの予定向けに、別の公式 LINE（新規アカウント）へメッセージを転送する。
 * uid を指定して ProLine フォーム（form9 フィールド）へ本文を書き込む。
 * デプロイ後の Web アプリ URL を PROLINE_NEW_MESSAGE_SENDER_URL 環境変数に設定すること。
 */
var FORM_URL = "https://ks215tqw.autosns.app/fm/bU7dgK2ysL";
var FIELD_KEY = "form9";
...
function doGet() {
  return jsonOutput_({
    ok: true,
    message: "ProLine new-account sender is running."
  });
}
 ...
 * @param {string} messageContent フォーム項目（form9）へ流し込む文章
 * @return {Object} 実行結果
 */
...
/**
 * 動作確認用のサンプル。userId / messageContent を実値に変えて使う。
 */
function testSendProlineFormMessage() {
  var userId = "USER_ID";
  var messageContent = "本日の面談リンクをお送りします。";
