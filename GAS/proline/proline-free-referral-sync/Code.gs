/**
 * このコードは、ProLine の友だち追加 GET とフォーム回答 POST を 1 本の Web アプリで受ける。
 * 友だち追加情報の upsert、form_8 シートへの追記、free2 からの紹介者名補完、1S の重複判定をまとめて行う。
 */
var SETTINGS_ = {
  mainSpreadsheetId: "1BQOswkbIBjzMdya5MICAMBcMmrJWd7rXLANPbSDc5m4",
  friendSheetName: "友達追加情報",
...
function doGet(e) {
  ...
  try {
    ...
    Logger.log("proline-unified-sync GET error: " + (error && error.stack ? error.stack : error));
...
function doPost(e) {
  ...
  try {
    ...
    setRowValue_(row, headerMap, "新旧", "旧");
    ...
    Logger.log("proline-unified-sync POST error: " + (error && error.stack ? error.stack : error));
    return ContentService.createTextOutput("Error: " + error.message);
  } finally {
    if (lock.hasLock()) {
      lock.releaseLock();
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu("Webhook Tools")
    .addItem("form_8 の重複チェック", "runForm8DuplicateCheck")
    .addToUi();
}

function runForm8DuplicateCheck() {
  var spreadsheet = openMainSpreadsheet_();
  var sheet = spreadsheet.getSheetByName(SETTINGS_.formSheetName);
  if (!sheet) {
    throw new Error("対象シートが見つかりません: " + SETTINGS_.formSheetName);
  }

  var headerMap = ensureHeaderMap_(sheet, FORM_HEADERS_);
  var result = applyDuplicateStatusToFormSheet_(sheet, headerMap);

  SpreadsheetApp.getUi().alert(
    "重複チェック完了\n" +
    "対象行: " + result.targetRows + "\n" +
    "重複: " + result.duplicateRows + "\n" +
    "1S予定: " + result.plannedRows + "\n" +
    "維持: " + result.keptRows
  );
}
...
function applyDuplicateStatusToFormSheet_(sheet, headerMap) {
  var uidColumn = getHeaderColumn_(headerMap, "uid");
  var statusColumn = getHeaderColumn_(headerMap, "1S");
  var ownerColumn = getHeaderColumn_(headerMap, SETTINGS_.ownerHeaderText);

  if (!uidColumn || !statusColumn) {
    throw new Error("uid 列または 1S 列が見つかりません。1行目を確認してください。");
  }
  ...
  var width = Math.max(sheet.getLastColumn(), FORM_HEADERS_.length);
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    var uid = normalizeString_(currentRow[uidColumn - 1]);
    var currentStatus = normalizeString_(currentRow[statusColumn - 1]);
    var nextStatus = currentStatus;
    var nextOwner = ownerColumn ? normalizeString_(currentRow[ownerColumn - 1]) : "";
    ...
    if (uid) {
      ...
      if (seen[uid].hasActiveExisting) {
        nextStatus = SETTINGS_.duplicateStatusText;
        duplicateRows += 1;
      } else if (!currentStatus || currentStatus === SETTINGS_.duplicateStatusText) {
        nextStatus = SETTINGS_.plannedStatusText;
        nextOwner = SETTINGS_.defaultOwnerText;
        seen[uid].hasActiveExisting = true;
        plannedRows += 1;
      } else if (isActiveFirstStepStatus_(currentStatus)) {
        seen[uid].hasActiveExisting = true;
        keptRows += 1;
      } else {
        keptRows += 1;
    ...
    if (ownerColumn) {
      ownerValues.push([nextOwner]);
    }
  }

  sheet.getRange(2, statusColumn, statusValues.length, 1).setValues(statusValues);
  if (ownerColumn) {
    sheet.getRange(2, ownerColumn, ownerValues.length, 1).setValues(ownerValues);
  }
